...
def main() -> int:
    ...
    if len(sys.argv) != 2:
        print("Usage: python -m kachaka_navigation.scripts.undock_shelf_at_location <LOCATION_ID>")
        return 1
    ...
    try:
        ...
        print(robot.move_to_location(location_id))
        ...
        logger.error("undock_shelf_at_location failed: %s", error)
        print()
        print("ERROR: Could not move to the requested location and undock the shelf.")
        print()
        print("Checklist:")
        ...
        print("5. The location has enough space to unload the shelf.")
        print()
